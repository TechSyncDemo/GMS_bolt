...
export default function Home() {
  ...
  const filteredEvents = mockEvents.filter(event => {
    ...
    const matchesPrice = priceFilter === 'all' || 
                        ...
                        (priceFilter === '1000-2000' && Math.min(...event.pricing.map(p => p.price)) >= 1000 && Math.min(...event.pricing.map(p => p.price)) <= 2000) ||
  ...
  const handleWhatsAppClick = (whatsapp: string, eventName: string) => {
    const message = encodeURIComponent(`Hi! I'm interested in booking tickets for ${eventName}. Can you help me?`);
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <nav className="hidden md:flex items-center space-x-6">
              ...
              <Link href="/organizer" className="text-gray-600 hover:text-purple-600 transition-colors">
                For Organizers
              </Link>
              <Link href="/auth" className="text-gray-600 hover:text-purple-600 transition-colors">
                Login
              </Link>
      ...
      <section className="py-20 px-4">
        <div className="container mx-auto text-center">
          ...
          <p className="text-xl text-gray-600 mb-8 max-w-2xl mx-auto">
            Discover the hottest events, concerts, and experiences. From indie gigs to massive festivals - we've got your scene covered.
          </p>
          <div className="flex flex-col md:flex-row gap-4 max-w-2xl mx-auto">
            ...
            <Button size="lg" className="bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 h-12 px-8">
              Find Events
            </Button>
          </div>
        </div>
        ...
        <div className="container mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredEvents.map((event) => (
              <Card key={event.id} className="overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-2 bg-white/80 backdrop-blur-sm border-0 shadow-lg">
